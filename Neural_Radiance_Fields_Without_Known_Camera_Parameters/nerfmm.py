...
def test(
    ...
    for i in range(int(np.ceil(H / chunk_size))):
        ray_origins_ = ray_origins[i * W * chunk_size:(i + 1) * W *
                                   chunk_size].to(camera_intrinsics.device)
        ray_directions_ = ray_directions[i * W * chunk_size:(i + 1) * W *
                                         chunk_size].to(
                                             camera_intrinsics.device)
        regenerated_px_values = render_rays(model,
                                            ray_origins_,
                                            ray_directions_,
                                            hn=hn,
                                            hf=hf,
...
class NerfModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.block3 = nn.Sequential(
            nn.Linear(embedding_dim_direction * 6 + hidden_dim + 3,
                      hidden_dim // 2),
            nn.ReLU(),
        )
...
def render_rays(nerf_model,
    ...
    t = torch.linspace(hn, hf, nb_bins,
                       device=device).expand(ray_origins.shape[0], nb_bins)
    mid = (t[:, :-1] + t[:, 1:]) / 2.0
    ...
    x = ray_origins.unsqueeze(1) + t.unsqueeze(2) * ray_directions.unsqueeze(
        1)  # [batch_size, nb_bins, 3]
    ray_directions = ray_directions.expand(nb_bins, ray_directions.shape[0],
                                           3).transpose(0, 1)

    colors, sigma = nerf_model(x.reshape(-1, 3), ray_directions.reshape(-1, 3))
    alpha = 1 - torch.exp(
        -sigma.reshape(x.shape[:-1]) * delta)  # [batch_size, nb_bins]
...
def train(
    ...
    for _ in tqdm(range(nb_epochs)):
        ...
        for img_index in ids:
            ...
            gt_px_values = torch.from_numpy(training_images[samples_idx]).to(
                camera_intrinsics.device)
            regenerated_px_values = render_rays(nerf_model,
                                                rays_o,
                                                rays_d,
                                                hn=hn,
                                                hf=hf,
...
def initialize_camera_parameters(images, device="cpu"):
    ...
    camera_extrinsics = torch.zeros((images.shape[0], 6),
                                    device=device,
                                    dtype=torch.float32,
                                    requires_grad=True)
...
def sample_batch(
    ...
    if sample_all:
        ...
        u, v = np.meshgrid(np.linspace(0, W - 1, W, dtype=int),
                           np.linspace(0, H - 1, H, dtype=int))
        u = torch.from_numpy(u.reshape(-1)).to(camera_intrinsics.device)
        v = torch.from_numpy(v.reshape(-1)).to(camera_intrinsics.device)
    else:
        image_indices = (torch.zeros(batch_size) + img_index).type(
            torch.long)  # Sample random images
        u = torch.randint(
            W, (batch_size,),
            device=camera_intrinsics.device)  # Sample random pixels
    ...
    phi_skew = torch.stack(
        [
            ...
            torch.cat([-r[1:2], r[0:1],
                       torch.zeros(1, device=r.device)]),
        ],
        dim=0,
    )
    alpha = r.norm() + 1e-15
    ...
    rays_d_world = torch.matmul(c2w[:3, :3].view(1, 3, 3),
                                rays_d_cam.unsqueeze(2)).squeeze(2)
    rays_o_world = c2w[:3, 3].view(1, 3).expand_as(rays_d_world)
    rays_o_world, rays_d_world = get_ndc_rays(H,
                                              W,
                                              focal,
                                              rays_o=rays_o_world,
                                              rays_d=rays_d_world)
...
if __name__ == "__main__":
    ...
    test(
        model,
        camera_intrinsics,
        (0.5 * camera_extrinsics[0] + 0.5 * camera_extrinsics[1]).unsqueeze(0),
        0.0,
        1.0,
        training_images,
        img_index=0,
