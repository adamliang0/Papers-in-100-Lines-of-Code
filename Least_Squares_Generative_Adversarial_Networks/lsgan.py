...
def get_minibatch(batch_size, n_mixture=8):
    ...
    mu_x, mu_y = torch.sin(thetas).reshape(-1,
                                           1), torch.cos(thetas).reshape(-1, 1)
    idx = torch.randint(n_mixture,
                        (batch_size,))  # Sample randomly a mixture component
    m = MultivariateNormal(torch.cat(([mu_x[idx], mu_y[idx]]), dim=1),
                           torch.eye(2) * 0.01**2)
    return m.sample()
...
def train(
    ...
    for epoch in tqdm(range(nb_epochs)):
        ...
        loss = 0.5 * mse_loss(d_score, torch.ones_like(d_score))
