...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim=784, output_dim=10):
        ...
        self.layers = nn.Sequential(nn.Linear(input_dim, input_dim), nn.ELU(),
...
class WeightDecay(nn.Module):
    ...
    def __init__(self, model, device):
        ...
        for m in model.parameters():
            self.parameter_dict[str(idx)] = torch.nn.Parameter(
                torch.rand(m.shape, device=device))
            idx += 1
    ...
    def forward(self, model):
        ...
        for coefficients, weights in zip(self.parameters(), model.parameters()):
            regularization += (self.positive_constraint(coefficients) *
                               weights**2).sum()
...
def gradient_based_ho(nb_epochs=int(10e3), N=50, device="cpu", batch_size=32):
    ...
    meta_optimizer = torch.optim.RMSprop(meta_model.parameters(), lr=0.1)
    ...
    for epoch in tqdm(range(nb_epochs)):  # Outer optimization loop
        for k in range(N):  # Inner optimization loop
            ...
            training_loss = compute_loss(
                trainX[batch_idx].to(device),
                trainY[batch_idx].to(device),
                model,
                meta_model,
            )
        ...
        training_loss = compute_loss(trainX.to(device), trainY.to(device),
                                     model, meta_model)
        validation_loss = compute_loss(valX.to(device), valY.to(device), model,
                                       meta_model)

        accuracies["training"].append(
            compute_pred(trainX.to(device), trainY.to(device), model))
        accuracies["validation"].append(
            compute_pred(valX.to(device), valY.to(device), model))

        hyper_grads = hypergradient(validation_loss, training_loss,
                                    meta_model.parameters, model.parameters)
...
if __name__ == "__main__":
    ...
    plt.plot(
        1 - np.array(accuracies["training"])[::5],
        alpha=0.8,
        linestyle="--",
        linewidth=2.5,
        label="Training",
    ...
    plt.plot(
        1 - np.array(accuracies["validation"])[::5],
        alpha=0.8,
        c="C0",
        linewidth=2.5,
        label="Validation",
