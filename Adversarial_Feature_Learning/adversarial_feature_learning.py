...
def get_minibatch(batch_size, device):
    ...
    return (torch.tensor(trainX[indices],
                         dtype=torch.float).reshape(batch_size, -1).to(device))
...
class G(nn.Module):
    ...
    def __init__(self):
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(1024, 1024),
            nn.BatchNorm1d(1024, eps=1e-05, momentum=0.1, affine=False),
            nn.ReLU(),
            nn.Linear(1024, 784),
            nn.Sigmoid(),
        )
...
def train(D,
    ...
    for epoch in tqdm(range(nb_epochs)):
        ...
        lambda_objective = torch.nn.BCELoss()(D(
            G(z), z).reshape(batch_size), torch.ones(
                batch_size, device=device)) + torch.nn.BCELoss()(
                    D(x, E(x)).reshape(batch_size),
...
if __name__ == "__main__":
    ...
    schedulers = [
        torch.optim.lr_scheduler.ExponentialLR(optimizer, 0.9999508793911394)
        for optimizer in optimizers
    ]
