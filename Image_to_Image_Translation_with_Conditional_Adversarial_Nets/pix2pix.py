...
class Dataset:
    ...
    def __getitem__(self, index):
        ...
        if (np.random.rand() > 0.5
           ):  # Make sure to apply the same transform to the input and output
            transform_list = [transforms.RandomHorizontalFlip(p=1.0)
                             ] + transform_list
...
def train(
    ...
    for epoch in tqdm(range(NB_EPOCHS)):
        for batch in dataloader:
            ...
            loss_G_GAN = torch.nn.functional.binary_cross_entropy_with_logits(
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(Dataset("datasets/facades/train"),
                            batch_size=1,
                            shuffle=True,
                            num_workers=0)
    train(
    ...
    netG.eval()
    test_dataloader = DataLoader(Dataset("datasets/facades/test"),
                                 batch_size=1,
                                 shuffle=True,
                                 num_workers=0)
    ...
    for idx, batch in enumerate(test_dataloader):
        ...
        output = netG(input)
        axes[idx,
             0].imshow(input[0].cpu().transpose(0, 1).transpose(1, 2).numpy() /
                       2.0 + 0.5)
        axes[idx, 0].axis("off")
        axes[idx, 1].imshow(
            ground_truth[0].cpu().transpose(0, 1).transpose(1, 2).numpy() /
            2.0 + 0.5)
        axes[idx, 1].axis("off")
        axes[idx, 2].imshow(
            output[0].data.cpu().transpose(0, 1).transpose(1, 2).numpy() / 2.0 +
            0.5)
        axes[idx, 2].axis("off")
        if idx == 3:
            break
