...
class Maxout(nn.Module):
    ...
    def forward(self, x):
        return torch.max(
            self.net(x).reshape(-1, self.k * self.dout).reshape(
                -1, self.dout, self.k),
            dim=-1,
        ).values


def train(model, optimizer, loss_fct, nb_epochs, batch_size, trainX, trainy,
          testX, testy):
    training_loss = []
    testing_accuracy = []
...
if __name__ == "__main__":
    ...
    model = torch.nn.Sequential(
        nn.Dropout(p=0.4),  # See col. 2 p. 2
        Maxout(28 * 28, 1200, k),
        nn.Dropout(p=0.4),
        Maxout(1200, 10, k),
        nn.LogSoftmax(dim=-1),
    ...
    training_loss, testing_accuracy = train(model, optimizer, loss_fct,
                                            nb_epochs, batch_size, trainX,
                                            trainy, testX, testy)

    ax = sns.lineplot(x=np.linspace(1, nb_epochs, nb_epochs),
                      y=testing_accuracy)
