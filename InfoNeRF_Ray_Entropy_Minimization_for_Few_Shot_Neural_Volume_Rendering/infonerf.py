...
def test(hn,
    ...
    for i in range(int(np.ceil(H / chunk_size))):
        ...
        regenerated_px_values, _ = render_rays(model,
                                               ray_origins_,
                                               ray_directions_,
                                               hn=hn,
                                               hf=hf,
...
class NerfModel(nn.Module):
    ...
    def __init__(self,
                 embedding_dim_pos=20,
                 embedding_dim_direction=8,
                 hidden_dim=128):
        ...
        self.block3 = nn.Sequential(
            nn.Linear(embedding_dim_direction * 3 + hidden_dim,
                      hidden_dim // 2),
            nn.ReLU(),
        )
...
def train(
    ...
    for _ in range(nb_epochs):
        for batch in tqdm(data_loader):
            ...
            regenerated_px_values, regularization = render_rays(
                nerf_model,
                ray_origins,
                ray_directions,
                hn=hn,
...
if __name__ == "__main__":
    ...
    training_dataset = np.concatenate((
        training_dataset[0 * 400 * 400:(0 + 1) * 400 * 400],
