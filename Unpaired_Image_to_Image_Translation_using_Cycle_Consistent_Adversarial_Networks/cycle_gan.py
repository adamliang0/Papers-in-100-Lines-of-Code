...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, ch):
        ...
        self.model = nn.Sequential(
            ...
            nn.Conv2d(ch, ch, kernel_size=3, padding=0, bias=True),
...
class Generator(nn.Module):
    ...
    def __init__(self, n_blocks=9):
        ...
        self.middle = nn.Sequential(
            *torch.nn.ModuleList([ResnetBlock(256) for _ in range(n_blocks)]))
...
class Dataset:
    ...
    def __init__(self, path_domain_x, path_domain_y):
        ...
        self.imgs_domain_x = [
            f for f in listdir(path_domain_x) if f.endswith(".jpg")
        ]
        self.imgs_domain_y = [
            f for f in listdir(path_domain_y) if f.endswith(".jpg")
        ]
    ...
    def __getitem__(self, index=None):
        ...
        A = Image.open(self.path_domain_x + "/" +
                       self.imgs_domain_x[index_x]).convert("RGB")
        B = Image.open(self.path_domain_y + "/" +
                       self.imgs_domain_y[index_y]).convert("RGB")
...
def training_loop(
    ...
    for _ in tqdm(range(nb_epochs)):
        for x, y in dataloader:
            ...
            d_score = Dx(fake_x)
            loss_g_yx = (
                nn.functional.mse_loss(d_score, torch.ones_like(d_score)) +
                lambda_ * cycle_loss_yxy +
                lambda_ * lambda_identity * idt_loss_yx)
...
if __name__ == "__main__":
    ...
    buffer_x = Buffer()
    buffer_y = Buffer()
    optimizer_generators = optim.Adam(list(G.parameters()) +
                                      list(F.parameters()),
                                      lr=0.0002,
                                      betas=(0.5, 0.999))
    optimizer_discriminators = optim.Adam(list(Dx.parameters()) +
                                          list(Dy.parameters()),
                                          lr=0.0002,
                                          betas=(0.5, 0.999))
    ...
    scheduler_g = lr_scheduler.LambdaLR(optimizer_generators,
                                        lr_lambda=lambda_rule)
    scheduler_d = lr_scheduler.LambdaLR(optimizer_discriminators,
                                        lr_lambda=lambda_rule)

    dataloader = DataLoader(
        Dataset("horse2zebra/horse2zebra/trainA",
                "horse2zebra/horse2zebra/trainB"),
        batch_size=1,
    )
    training_loop(
        G,
    ...
    original = (torch.from_numpy(
        np.array(
            Image.open("horse2zebra/horse2zebra/testA/n02381460_1300.jpg"))) /
                255.0)
    ...
    axes[0, 1].imshow(
        g_x.transpose(2, 3).transpose(1, 3)[0].data.cpu().numpy() * 0.5 + 0.5)
    axes[0, 1].set_title("G(x)")
    axes[0, 2].imshow(
        f_g_x.transpose(2, 3).transpose(1, 3)[0].data.cpu().numpy() * 0.5 + 0.5)
    axes[0, 2].set_title("F(G(x))")

    original = (torch.from_numpy(
        np.array(
            Image.open("horse2zebra/horse2zebra/testB/n02391049_5100.jpg"))) /
                255.0)
    ...
    axes[1, 1].imshow(
        f_x.transpose(2, 3).transpose(1, 3)[0].data.cpu().numpy() * 0.5 + 0.5)
    axes[1, 1].set_title("F(x)")
    axes[1, 2].imshow(
        g_f_x.transpose(2, 3).transpose(1, 3)[0].data.cpu().numpy() * 0.5 + 0.5)
