...
def get_minibatch(batch_size, device):
    ...
    return torch.tensor(trainX[indices], dtype=torch.float).reshape(
        batch_size, -1).to(device), torch.nn.functional.one_hot(
            torch.tensor(trainy[indices], dtype=torch.long),
            num_classes=10).to(device).type(torch.float)
...
class Generator(nn.Module):
    ...
    def forward(self, noise, context):
        h = torch.cat((self.hidden1_z(noise), self.hidden1_context(context)),
                      dim=1)
        h = self.hidden2(h)
    ...
    for epoch in tqdm(range(nb_epochs)):
        ...
        for _ in range(k):
            ...
            f_loss = torch.nn.BCELoss()(
                discriminator(generator(z, label), label).reshape(batch_size),
                torch.zeros(batch_size, device=device),
            )
            r_loss = torch.nn.BCELoss()(
                discriminator(x, label).reshape(batch_size),
...
if __name__ == "__main__":
    ...
    schedulers = [
        torch.optim.lr_scheduler.ExponentialLR(optimizer_d, 1 / 1.00004),
    ...
    for i in range(10):
        ...
        context = (torch.nn.functional.one_hot(
            torch.ones(NB_IMAGES, dtype=torch.long) * i,
            num_classes=10).to(device).type(torch.float))
