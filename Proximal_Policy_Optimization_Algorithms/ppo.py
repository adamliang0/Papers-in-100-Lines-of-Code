...
class Environments:
    ...
    def reset_env(self, env_id):
        ...
        for _ in range(random.randint(
            ...
            self.observations[env_id], reward, _, info = self.envs[env_id].step(
                1)
...
def PPO(
    ...
    for iteration in tqdm(range(nb_iterations)):
        advantages = torch.zeros((envs.len(), T),
                                 dtype=torch.float32,
                                 device=device)
        buffer_states = torch.zeros((envs.len(), T, 4, 84, 84),
                                    dtype=torch.float32,
                                    device=device)
        buffer_actions = torch.zeros((envs.len(), T),
                                     dtype=torch.long,
                                     device=device)
        buffer_logprobs = torch.zeros((envs.len(), T),
                                      dtype=torch.float32,
                                      device=device)
        buffer_state_values = torch.zeros((envs.len(), T + 1),
                                          dtype=torch.float32,
                                          device=device)
        buffer_rewards = torch.zeros((envs.len(), T),
                                     dtype=torch.float32,
                                     device=device)
        buffer_is_terminal = torch.zeros((envs.len(), T),
                                         dtype=torch.float16,
                                         device=device)
        ...
        for env_id in range(envs.len()):
            with torch.no_grad():
                for t in range(
                        T):  # Run policy θ_old in environment for T timesteps
                    obs = (torch.from_numpy(
                        envs.observations[env_id] /
                        255.0).unsqueeze(0).float().to(device))
                    ...
                    buffer_actions[env_id,
                                   t] = torch.tensor([action]).to(device)
                    ...
                    if dead:
                        if envs.total_rewards[env_id] > max_reward:
                            ...
                            torch.save(actorcritic.cpu(),
                                       f"actorcritic_{max_reward}")
                ...
                buffer_state_values[env_id, T] = actorcritic(
                    torch.from_numpy(
                        envs.observations[env_id] /
                        255.0).unsqueeze(0).float().to(device))[1].squeeze(0)

                # Compute advantage estimates A^1; ... ; A^T
                for t in range(T - 1, -1, -1):
                    next_non_terminal = 1.0 - buffer_is_terminal[env_id, t]
                    delta_t = (buffer_rewards[env_id, t] +
                               gamma * buffer_state_values[env_id, t + 1] *
                               next_non_terminal -
                               buffer_state_values[env_id, t])
                    if t == (T - 1):
                        A_t = delta_t
                    else:
                        A_t = (delta_t + gamma * gae_parameter *
                               advantages[env_id, t + 1] * next_non_terminal)
        ...
        for epoch in range(K):
            advantages_data_loader = DataLoader(
                TensorDataset(
                    advantages.reshape(advantages.shape[0] *
                                       advantages.shape[1]),
                    buffer_states.reshape(
                        -1,
                        buffer_states.shape[2],
                        buffer_states.shape[3],
            ...
            for batch_advantages in advantages_data_loader:
                b_adv, obs, action_that_was_taken, old_log_prob, old_state_values = (
                    batch_advantages)
                ...
                policy_loss_2 = b_adv * torch.clamp(ratio, 1 - clip_range,
                                                    1 + clip_range)
                ...
                value_loss2 = F.mse_loss(
                    returns,
                    torch.clamp(value, value - clip_range, value + clip_range),
                    reduction="none",
                )
                value_loss = torch.max(value_loss1, value_loss2).mean()

                loss = (policy_loss + ent_coef_c2 * -(m.entropy()).mean() +
                        vf_coeff_c1 * value_loss)
