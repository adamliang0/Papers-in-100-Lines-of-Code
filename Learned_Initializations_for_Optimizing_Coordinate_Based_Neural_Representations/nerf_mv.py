...
class NerfModel(nn.Module):
    ...
    def forward(self, o):
        emb_x = torch.cat(
            [
                torch.cat([torch.sin(o * (2**i)),
                           torch.cos(o * (2**i))],
                          dim=-1) for i in torch.linspace(0, 8, 20)
            ],
            axis=-1,
        )
        h = self.net(emb_x)
        ...
        if os.path.isfile(transforms):
            ...
            for view_idx in range(N):
                ...
                img = torch.from_numpy(
                    imread(scene_path +
                           f"/{view['file_path'].split('/')[-1]}.png") / 255.0)
                c2w = torch.tensor(view["transform_matrix"])

                focal_length = (
                    W / 2.0 /
                    torch.tan(torch.tensor(data["camera_angle_x"]) / 2.0))
                u, v = torch.meshgrid(torch.arange(W), torch.arange(H))
                dirs = torch.stack(
                    (v - W / 2, -(u - H / 2),
                     -torch.ones_like(u) * focal_length),
                    axis=-1,
                )
                dirs = (c2w[:3, :3] @ dirs[..., None]).squeeze(-1)
                scene_rays_d[view_idx] = dirs / np.linalg.norm(
                    dirs, axis=-1, keepdims=True)
                scene_rays_o[view_idx] = (
                    torch.zeros_like(scene_rays_d[view_idx]) + c2w[:3, 3])
                scene_gt_pixels[view_idx] = (img[..., :3] * img[..., -1:] + 1 -
                                             img[..., -1:])
...
def sample_task(rays_o, rays_d, gt_pixels):
    ...
    return torch.cat([o.reshape(-1, 3),
                      d.reshape(-1, 3),
                      gt.reshape(-1, 3)],
                     dim=-1)


def perform_k_training_steps(
    nerf_model,
    task,
    k,
    optimizer,
    ...
    for _ in range(k):
        ...
        regenerated_px_values = render_rays(nerf_model,
                                            ...
                                            hf=hf,
                                            nb_bins=nb_bins)
        loss = nn.functional.mse_loss(ground_truth_px_values,
...
def reptile(
    ...
    for epoch in tqdm(range(nb_iterations)):
        ...
        phi_tilde = perform_k_training_steps(nerf_model,
                                             task,
                                             k,
                                             optimizer,
                                             device=device)
...
if __name__ == "__main__":
    ...
    rays_o, rays_d, gt_pixels = load_data("data/cars/",
                                          "data/car_splits.json",
                                          train=True,
                                          N=25,
                                          H=128,
                                          W=128)
    reptile(
        ...
        100_000,
        device,
        lambda: sample_task(rays_o, rays_d, gt_pixels),
    ...
    for test_img in range(10):
        ...
        test_data = torch.cat(
            [
                rays_o[test_img][0].reshape(-1, 3),
                rays_d[test_img][0].reshape(-1, 3),
                gt_pixels[test_img][0].reshape(-1, 3),
            ],
            dim=-1,
        ).to(device)

        training_loss = perform_k_training_steps(nerf_model,
                                                 test_data,
                                                 1000,
                                                 optimizer,
                                                 batch_size=128,
        ...
        with torch.no_grad():
            for idx, i in enumerate(range(1, 25, 3)):
                ...
                plt.axis("off")
                plt.imshow(
                    img.reshape(128, 128, 3).data.cpu().numpy().clip(0, 1))
                if (test_img == 0) and (idx == 3):
                    plt.title("Novel views")
