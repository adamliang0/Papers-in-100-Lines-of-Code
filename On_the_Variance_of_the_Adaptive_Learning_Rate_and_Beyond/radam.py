...
class RAdam:
    ...
    def step(self):
        ...
        self.mt = [
            ...
            for p, m in zip(model.parameters(), self.mt)
        ]
        self.vt = [
            self.beta2 * v + (1 - self.beta2) * p.grad**2
            for p, v in zip(model.parameters(), self.vt)
        ]
        m_hat_t = [m / (1 - self.beta1**self.t) for m in self.mt]

        pho_t = self.rho_inf - 2 * self.t * self.beta2**self.t / (
            1 - self.beta2**self.t)
        if pho_t > 4:
            lt = [
                torch.sqrt((1 - self.beta2**self.t) / (v + self.epsilon))
                for v in self.vt
            ]
            rt = np.sqrt(((pho_t - 4) * (pho_t - 2) * self.rho_inf) /
                         ((self.rho_inf - 4) * (self.rho_inf - 2) * pho_t))
