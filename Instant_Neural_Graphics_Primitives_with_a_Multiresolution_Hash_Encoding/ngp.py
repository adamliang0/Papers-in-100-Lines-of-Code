...
class NGP(torch.nn.Module):
    ...
    def forward(self, x, d):
        ...
        for i, N in enumerate(self.Nl):
            ...
            h_x = torch.remainder(torch.bitwise_xor(torch.bitwise_xor(a, b), c),
                                  self.T)
            ...
            features[:, i * 2:(i + 1) * 2] = (torch.nn.functional.grid_sample(
                volume,
                ((x[mask] * N - floor) -
                 0.5).unsqueeze(1).unsqueeze(1).unsqueeze(1),
...
def train(
    ...
    for _ in range(nb_epochs):
        for batch in tqdm(data_loader):
            ...
            pred_px_values = render_rays(nerf_model,
                                         ray_origins,
                                         ray_directions,
                                         hn=hn,
                                         hf=hf,
...
if __name__ == "__main__":
    ...
    training_dataset = torch.from_numpy(
        np.load("training_data_800x800.pkl", allow_pickle=True))
    testing_dataset = torch.from_numpy(
        np.load("testing_data_800x800.pkl", allow_pickle=True))
    L = 16
    F = 2
    T = 2**19
    N_min = 16
