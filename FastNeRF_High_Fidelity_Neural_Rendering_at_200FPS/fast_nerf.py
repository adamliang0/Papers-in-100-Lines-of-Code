...
class FastNerf(nn.Module):
    ...
    def forward(self, o, d):
        sigma_uvw = self.Fpos(
            self.positional_encoding(o, self.embedding_dim_pos))
        sigma = torch.nn.functional.softplus(
            sigma_uvw[:, 0][..., None])  # [batch_size, 1]
        uvw = torch.sigmoid(sigma_uvw[:,
                                      1:].reshape(-1, 3,
                                                  self.D))  # [batch_size, 3, D]

        beta = torch.softmax(
            self.Fdir(self.positional_encoding(d,
                                               self.embedding_dim_direction)),
            -1)
...
class Cache(nn.Module):
    ...
    def forward(self, x, d):
        ...
        # Position
        idx = ((x[mask] / (self.scale / self.Np) + self.Np / 2).long().clip(
            0, self.Np - 1))
...
def test(model, hn, hf, dataset, img_index=0, nb_bins=192, H=400, W=400):
    ...
    plt.figure()
    plt.imshow(regenerated_px_values.data.cpu().numpy().reshape(H, W,
                                                                3).clip(0, 1))
    plt.axis("off")
