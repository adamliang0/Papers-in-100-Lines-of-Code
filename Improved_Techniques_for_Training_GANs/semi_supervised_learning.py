...
class Generator(nn.Module):
    ...
    def __init__(self, latent_dim=100, output_dim=28 * 28):
        ...
        self.network = nn.Sequential(
            ...
            nn.Softplus(),
            nn.BatchNorm1d(500),
            nn.utils.weight_norm(nn.Linear(500, output_dim)),
            nn.Sigmoid(),
        )
...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.network_head = nn.Sequential(
            GaussianNoiseLayer(sigma=0.3),
            nn.utils.weight_norm(nn.Linear(28**2, 1000)),
            nn.ReLU(),
            GaussianNoiseLayer(sigma=0.5),
        ...
        self.network_tail = nn.Sequential(
            GaussianNoiseLayer(sigma=0.5),
            nn.utils.weight_norm(nn.Linear(250, 10)))
...
def train(
    ...
    for epoch in tqdm(range(nb_epochs)):
        ...
        noise = torch.rand((supervised_data.shape[0], latent_dim),
                           device=device)
        fake_data = g(noise)
        # Supervised loss
        log_prob = torch.nn.functional.log_softmax(d(
            supervised_data.to(device))[1],
                                                   dim=1)
        supervised_loss = torch.nn.functional.nll_loss(log_prob,
                                                       target.to(device))
        # Unsupervised loss
        _, prob_before_softmax_unsupervised = d(unsupervised_data)
        _, prob_before_softmax_fake = d(fake_data)
        unsupervised_loss = (
            0.5 * torch.nn.functional.softplus(
                torch.logsumexp(prob_before_softmax_fake, dim=1)).mean() - 0.5 *
            torch.logsumexp(prob_before_softmax_unsupervised, dim=1).mean() +
            0.5 * torch.nn.functional.softplus(
                torch.logsumexp(prob_before_softmax_unsupervised,
                                dim=1)).mean())
        ...
        noise = torch.rand((unsupervised_data.shape[0], latent_dim),
                           device=device)
        fake_data = g(noise)
        ...
        testing_accuracy.append(
            (log_prob.argmax(-1)
             == torch.from_numpy(testy).to(device)).sum().item() /
            testy.shape[0])
        d.train(mode=True)
...
if __name__ == "__main__":
    ...
    trainy_sup, trainy_unsup, trainX_sup, trainX_unsup, testX, testy = (
        generate_training_dataset(100))
    sup_data_loader = DataLoader(
        TensorDataset(
            torch.from_numpy(trainX_sup.reshape(-1, 28 * 28)),
            torch.from_numpy(trainy_sup.reshape(-1)),
        ),
        batch_size=100,
        shuffle=True,
    )
    unsup_data_loader1 = DataLoader(
        torch.from_numpy(trainX_unsup.reshape(-1, 28 * 28)),
        batch_size=100,
        shuffle=True,
    )
    unsup_data_loader2 = DataLoader(
        torch.from_numpy(trainX_unsup.reshape(-1, 28 * 28)),
        batch_size=100,
        shuffle=True,
    )
