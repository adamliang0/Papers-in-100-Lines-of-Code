...
def train_rectified_flow(rectified_flow,
    ...
    for epoch in tqdm(range(NB_EPOCHS)):
        for z0, z1 in train_dataloader:
            ...
            loss = ((target - pred).view(pred.shape[0],
                                         -1).abs().pow(2).sum(dim=1).mean())
...
def sample(rectified_flow, T, pi_0):
    ...
    for i in range(T):
        ...
        samples.append((samples[-1] + drift_pred * 1.0 / T))
...
if __name__ == "__main__":
    ...
    theta = np.array([0.0, 60, 120, 180, 240, 300]) / 360 * 2 * np.pi
    std = 0.5
    radius = 12.0
    modes = np.array([
        (radius * x, radius * y) for x, y in zip(np.cos(theta), np.sin(theta))
    ])
    ...
    optimizer = torch.optim.Adam(rectified_flow.parameters(),
                                 lr=5e-3,
                                 weight_decay=wd)
    ...
    train_dataloader = DataLoader(dataset,
                                  ...
                                  shuffle=True,
                                  num_workers=0)
    training_loss = train_rectified_flow(rectified_flow,
                                         optimizer,
                                         train_dataloader,
                                         nb_epochs,
                                         eps=1e-15)
    ...
    for idx, theta_ in enumerate([theta[::2], theta[1::2]]):
        modes = np.array([
            (12.0 * x, 12.0 * y) for x, y in zip(np.cos(theta_), np.sin(theta_))
        ])

        test_pi_0 = sample_multimodal_distribution(modes, std, batch_size=1000)
        test_pi_1 = sample(rectified_flow, 1000,
                           torch.from_numpy(test_pi_0).to(device))
        plt.scatter(test_pi_0[:, 0], test_pi_0[:, 1], c="purple")
        plt.scatter(
            test_pi_1[-1, :, 0].data.cpu().numpy(),
            test_pi_1[-1, :, 1].data.cpu().numpy(),
            c="red",
        )
