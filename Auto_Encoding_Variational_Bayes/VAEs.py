...
trainX = (
    torch.tensor(img_data[:int(0.8 * img_data.shape[0])], dtype=torch.float) /
    255.0)
...
class Model(nn.Module):
    ...
    def compute_log_density(self, y, x):
        ...
        """
        mu, log_var = self.get_mean_and_log_var(x)
        log_density = -0.5 * (torch.log(2 * torch.tensor(np.pi)) + log_var +
                              (((y - mu)**2) /
                               (torch.exp(log_var) + 1e-10))).sum(dim=1)
        return log_density
...
def AVEB(
    ...
    for epoch in tqdm(range(nb_epochs)):
        ...
        epsilon = torch.normal(torch.zeros(M * L, latent_dim),
...
if __name__ == "__main__":
    device = "cuda:0"
    encoder = Model(data_dim=2,
                    context_dim=img_size[0] * img_size[1],
                    hidden_dim=200).to(device)
    decoder = Model(
        data_dim=img_size[0] * img_size[1],
        context_dim=2,
        hidden_dim=200,
        constrain_mean=True,
    ).to(device)
    encoder_optimizer = torch.optim.Adagrad(encoder.parameters(),
                                            lr=0.01,
                                            weight_decay=0.5)
    ...
    plt.plot(100 * np.arange(len(loss)),
             -np.array(loss),
             c="r",
             label="AEVD (train)")
    plt.xscale("log")
    ...
    grid_size = 10
    xx, yy = norm.ppf(
        np.meshgrid(np.linspace(0.1, 0.9, grid_size),
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            img = decoder.get_mean_and_log_var(
                torch.tensor([[xx[i, j], yy[i, j]]],
                             device=device,
                             dtype=torch.float))
            ax = fig.add_subplot(grid[i, j])
            ax.imshow(
                np.clip(
                    img[0].data.cpu().numpy().reshape(img_size[0], img_size[1]),
                    0, 1),
                cmap="gray",
                aspect="auto",
            )
