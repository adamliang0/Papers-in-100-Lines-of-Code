...
@torch.no_grad()
def sample_task():
    a = torch.rand(1).item() * 4.9 + 0.1  # Sample the amplitude in [0.1, 5.0]
...
if __name__ == "__main__":
    ...
    params = [
        torch.rand(40, 1,
                   device=device).uniform_(-np.sqrt(6.0 / 41),
                                           np.sqrt(6.0 / 41)).requires_grad_(),
        torch.zeros(40, device=device).requires_grad_(),
        torch.rand(40, 40,
                   device=device).uniform_(-np.sqrt(6.0 / 80),
                                           np.sqrt(6.0 / 80)).requires_grad_(),
        torch.zeros(40, device=device).requires_grad_(),
        torch.rand(1, 40,
                   device=device).uniform_(-np.sqrt(6.0 / 41),
                                           np.sqrt(6.0 / 41)).requires_grad_(),
