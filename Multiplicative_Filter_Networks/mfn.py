...
class Siren(nn.Module):
    ...
    def __init__(self, w0=30, in_dim=2, hidden_dim=256, out_dim=1):
        ...
        with torch.no_grad():
            ...
            self.net[2].weight.uniform_(-np.sqrt(6.0 / hidden_dim) / w0,
                                        np.sqrt(6.0 / hidden_dim) / w0)
            self.net[4].weight.uniform_(-np.sqrt(6.0 / hidden_dim) / w0,
                                        np.sqrt(6.0 / hidden_dim) / w0)
            self.net[6].weight.uniform_(-np.sqrt(6.0 / hidden_dim) / w0,
                                        np.sqrt(6.0 / hidden_dim) / w0)
            self.net[8].weight.uniform_(-np.sqrt(6.0 / hidden_dim) / w0,
                                        np.sqrt(6.0 / hidden_dim) / w0)
...
class GaborFilter(nn.Module):
    ...
    def forward(self, x):
        ...
        norm = ((x**2).sum(dim=1).unsqueeze(-1) +
                (self.mu**2).sum(dim=1).unsqueeze(0) - 2 * x @ self.mu.T)
...
class GaborNet(nn.Module):
    ...
    def __init__(self, in_dim=2, hidden_dim=256, out_dim=1, k=4):
        ...
        for lin in self.linear[:k - 1]:
            lin.weight.data.uniform_(-np.sqrt(1.0 / hidden_dim),
...
if __name__ == "__main__":
    ...
    pixel_coordinates = torch.cat((x.reshape(-1, 1), y.reshape(-1, 1)),
                                  dim=1).to(device)
    ...
    for i, model in enumerate([siren, gabor_net]):
        # Training
        optim = torch.optim.Adam(lr=1e-4 if (i == 0) else 1e-2,
