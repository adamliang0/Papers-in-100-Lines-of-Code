...
class DiffusionModel:
    ...
    def dpm_solver_sampling(
        ...
        xT = torch.randn((n_samples, image_channels, img_size[0], img_size[1]),
                         device=self.device)
        x_tilde = xT
        ...
        for i in tqdm(range(n_steps), desc="DPM-Solver", disable=not use_tqdm):
            ...
            # midpoint in λ-space
            lam_mid = (self.lambdas[t_prev - 1] + self.lambdas[t_cur - 1]) / 2.0
            ...
            t_prev_tensor = torch.full((n_samples,),
                                       t_prev,
                                       dtype=torch.long,
                                       device=self.device)
            u_i = (self.alphas[s_i - 1] /
                   self.alphas[t_prev - 1]) * x_tilde - self.sigmas[s_i - 1] * (
                       torch.exp(h * 0.5) - 1) * self.function_approximator(
                           x_tilde, t_prev_tensor - 1)

            t_s_tensor = torch.full((n_samples,),
                                    s_i,
                                    dtype=torch.long,
                                    device=self.device)
            x_tilde = (self.alphas[t_cur - 1] / self.alphas[t_prev - 1]
                      ) * x_tilde - self.sigmas[t_cur - 1] * (
                          torch.exp(h) - 1) * self.function_approximator(
                              u_i, t_s_tensor - 1)
...
if __name__ == "__main__":
    ...
    nb_images = 81
    samples = diffusion.dpm_solver_sampling(n_samples=nb_images,
                                            n_steps=10,
                                            use_tqdm=True)
