    ...
    for epoch in tqdm(range(nb_epochs)):
        ...
        for _ in range(k):
            ...
            gradients = torch.autograd.grad(
                outputs=critic(x_hat).squeeze(0),
                inputs=x_hat,
                grad_outputs=torch.ones((x_hat.shape[0]), device=x_hat.device),
                create_graph=True,
                retain_graph=True,
            )[0]
            gradient_penalty = (
                (gradients.reshape(gradients.size(0), -1).norm(2, dim=1) -
                 1)**2).mean()
...
if __name__ == "__main__":
    ...
    data = DataLoader(Dataset("../data"),
                      ...
                      shuffle=True,
                      num_workers=0)
    loss = train(generator, discriminator, optimizer_g, optimizer_d, data,
                 25_000)
