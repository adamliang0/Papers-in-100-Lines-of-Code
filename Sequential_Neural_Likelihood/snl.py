...
def sample_from_mcmc(
    ...
    thinning=10,
    num_chains=10,
    transition_distribution=MultivariateNormalDistribution(),
    eps=1e-15,
    device="cpu",
):
    ...
    for t in tqdm(range(1, T * thinning), desc="Sampling from MCMC"):
        ...
        p_x_given_theta = likelihood_function(
            observation_x.repeat(num_chains, 1).to(device),
            theta_t.to(device)).cpu()
        p_x_given_theta_prime = likelihood_function(
            observation_x.repeat(num_chains, 1).to(device),
        ...
        acceptance_probability = (density_prior_prime * p_x_given_theta_prime *
                                  q_theta_prime_given_theta) / (
                                      density_prior * p_x_given_theta *
                                      q_theta_given_theta_prime + eps)
...
def train(estimator,
          ...
          theta_dim=5,
          batch_size=100,
          nb_epochs=1000):  # @Todo : improve. E.g. early stopping, ...
...
def snl(x_0,
        ...
        num_chains=10,
        device="cpu"):
    likelihood_estimator = lambda x, theta: torch.exp(
        estimator.compute_ll(x, context=theta)[0])

    dataset = torch.tensor([])
    for r in range(R):
        theta_n = (sample_from_mcmc(
            prior,
...
if __name__ == "__main__":
    ...
    likelihood_estimator = lambda x, theta: torch.exp(
        model.compute_ll(x, context=theta)[0])
    posterior_samples = sample_from_mcmc(
        prior_distribution,
        likelihood_estimator,
        observation,
        T=500,
    ...
    make_plot(
        posterior_samples.data.cpu().numpy(),
        "Imgs/posterior_samples.png",
        gt_parameters.data.numpy(),
    )
