...
class DiffusionModel:
    ...
    def ddim_sampling(
        ...
        for i in progress_bar(range(n_steps)):
            ...
            t_tensor = torch.ones(
                n_samples, dtype=torch.long, device=self.device) * t

            alpha_bar_t = (
                self.alpha_bar[t - 1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1))
            alpha_bar_prev = (
                self.alpha_bar[t - step_size -
                               1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                if t > step_size else torch.tensor(1.0).to(self.device))

            # Predicted noise
            eps_pred = self.function_approximator(x, t_tensor - 1)

            # Predicted x_0
            x0_pred = (x - torch.sqrt(1 - alpha_bar_t) *
                       eps_pred) / torch.sqrt(alpha_bar_t)
...
if __name__ == "__main__":
    model = torch.load(
        "model_ddpm_mnist"
    )  # Model from https://github.com/MaximeVandegar/Papers-in-100-Lines-of-Code/blob/main/Denoising_Diffusion_Probabilistic_Models
