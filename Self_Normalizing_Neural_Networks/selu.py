...
class SELU(nn.Module):
    ...
    def forward(self, x):
        return self.lambda_ * (
            torch.maximum(torch.tensor([0], device=x.device), x) +
...
def _init_weights(module, init_gain=1.0):
    if isinstance(module, nn.Linear):
        module.weight.data.normal_(mean=0.0,
                                   std=np.sqrt(init_gain /
...
if __name__ == "__main__":
    ...
    for depth in [8, 16, 32]:
        ...
        training_loss_snn[depth] = train(model,
                                         optimizer,
                                         dataset,
                                         device=device,
                                         nb_epochs=nb_epochs)
    ...
    for depth in [8, 16, 32]:
        ...
        model.apply(lambda x: _init_weights(x, init_gain=2.0))
        optimizer = torch.optim.SGD(model.parameters(), lr=1e-5)
        training_loss_mlp[depth] = train(model,
                                         optimizer,
                                         dataset,
                                         device=device,
                                         nb_epochs=nb_epochs)
    ...
    fontsize = 14
    plt.plot(training_loss_mlp[8],
             color=plt.cm.bwr([0.05]),
             label="BatchNorm Depth 8")
    plt.plot(training_loss_mlp[16],
             color=plt.cm.bwr([0.15]),
             label="BatchNorm Depth 16")
    plt.plot(training_loss_mlp[32],
             color=plt.cm.bwr([0.25]),
             label="BatchNorm Depth 32")
    plt.plot(training_loss_snn[8],
             color=plt.cm.bwr([0.75]),
             label="SNN Depth 8")
    plt.plot(training_loss_snn[16],
             color=plt.cm.bwr([0.85]),
             label="SNN Depth 16")
    plt.plot(training_loss_snn[32],
             color=plt.cm.bwr([0.95]),
             label="SNN Depth 32")
    plt.yscale("log")
