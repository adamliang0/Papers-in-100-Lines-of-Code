...
def eval_spherical_function(k, d):
    ...
    return (0.282095 * k[..., 0] + -0.488603 * y * k[..., 1] +
            0.488603 * z * k[..., 2] - 0.488603 * x * k[..., 3] +
            (1.092548 * x * y * k[..., 4] - 1.092548 * y * z * k[..., 5] +
             0.315392 * (2.0 * z * z - x * x - y * y) * k[..., 6] +
             -1.092548 * x * z * k[..., 7] + 0.546274 *
             (x * x - y * y) * k[..., 8]))
...
class NerfModel(nn.Module):
    ...
    def forward(self, x, d):
        ...
        idx = ((x[mask] / (2 * self.scale / self.N) + self.N / 2).long().clip(
            0, self.N - 1))
    ...
    for _ in range(nb_epochs):
        for batch in tqdm(data_loader):
            ...
            regenerated_px_values = render_rays(nerf_model,
                                                ...
                                                hf=hf,
                                                nb_bins=nb_bins)
            loss = torch.nn.functional.mse_loss(ground_truth_px_values,
