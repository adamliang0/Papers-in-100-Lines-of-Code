...
def gaussian_log_pdf(z):
    ...
    """
    return -0.5 * (torch.log(torch.tensor([math.pi * 2], device=z.device)) +
                   z**2).sum(1)
...
class PlanarFlow(nn.Module):
    ...
    def constrained_u(self):
        ...
        return self.u + (m(wu) - wu) * (self.w /
                                        (torch.norm(self.w)**2 + 1e-15))
...
def plot_flow_density(
    ...
    xx, yy = np.meshgrid(
        np.linspace(
            latent_space_boundaries[0][0],
            latent_space_boundaries[0][1],
            nb_point_per_dimension,
        ),
        np.linspace(
            latent_space_boundaries[1][0],
            latent_space_boundaries[1][1],
            nb_point_per_dimension,
        ),
    )
    z = torch.tensor(
    ...
    ax.pcolormesh(
        zk[:, 0].detach().data.cpu().reshape(nb_point_per_dimension,
                                             nb_point_per_dimension),
        zk[:, 1].detach().data.cpu().reshape(nb_point_per_dimension,
                                             nb_point_per_dimension) * -1,
        qk.detach().data.reshape(nb_point_per_dimension,
                                 nb_point_per_dimension),
        cmap=cmap,
        rasterized=True,
    )
...
def plot_exact_density(
    ...
    exact_log_density,
    lims=np.array([[-4, 4], [-4, 4]]),
    nb_point_per_dimension=100,
    cmap="coolwarm",
    title=None,
):
    xx, yy = np.meshgrid(
        np.linspace(lims[0][0], lims[0][1], nb_point_per_dimension),
        np.linspace(lims[1][0], lims[1][1], nb_point_per_dimension),
    )
    z = torch.tensor(
        np.concatenate((xx.reshape(-1, 1), yy.reshape(-1, 1)), axis=1))
    density = torch.exp(exact_log_density(z)).reshape(nb_point_per_dimension,
                                                      nb_point_per_dimension)
    ax.imshow(density,
...
# Energy functions
U1 = lambda z: (((torch.sqrt(z[:, 0]**2 + z[:, 1]**2) - 2) / 0.4)**2
               ) / 2 - torch.log(1e-15 + (torch.exp(-((
                   (z[:, 0] - 2) / 0.6)**2) / 2) + torch.exp(-((
                       (z[:, 0] + 2) / 0.6)**2) / 2)))
U2 = lambda z: (((z[:, 1] - w1(z)) / 0.4)**2) / 2
U3 = lambda z: -torch.log(1e-15 + torch.exp(-((
    (z[:, 1] - w1(z)) / 0.35)**2) / 2) + torch.exp(-((
        (z[:, 1] - w1(z) + w2(z)) / 0.35)**2) / 2))
U4 = lambda z: -torch.log(1e-15 + torch.exp(-((
    (z[:, 1] - w1(z)) / 0.4)**2) / 2) + torch.exp(-((
...
if __name__ == "__main__":
    ...
    for U in [U1, U2, U3, U4]:
        ...
        plt.xticks([], [])
        plt.yticks([], [])
        plot_exact_density(ax,
                           exact_log_density,
                           title=r"$\exp^{-U(z)}$" if index == 1 else None)
        index += 2

        for flow_length in [2, 8, 32]:
            flow = NormalizingFlow(flow_length, data_dim).to(device)
            optimizer = torch.optim.Adam(flow.parameters(), lr=1e-2)
            loss = train(flow, optimizer, 20000, exact_log_density, 4096,
                         data_dim)
            ...
            plot_flow_density(flow,
                              ax,
                              title=f"K={flow_length}" if index <= 5 else None)
            index += 1
