...
trainX = (np.float32(trainX) + torch.rand(trainX.shape).numpy()) / 255.0
...
if __name__ == "main":
    ...
    assert torch.allclose(normalizing_flow.invert(normalizing_flow(x)[0]),
                          ...
                          rtol=1e-04,
                          atol=1e-06)
    optimizer = torch.optim.Adam(normalizing_flow.parameters(),
                                 lr=0.0002,
                                 weight_decay=0.9)
    ...
    for i in range(nb_data):
        for j in range(nb_data):
            x = (normalizing_flow.invert(
                logistic_distribution.sample().unsqueeze(0)).data.cpu().numpy())
